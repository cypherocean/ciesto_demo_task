                    ...
                    <form  name="form" id="form" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="form-group col-sm-6">
                                ...
                                <select name="shop_id" id="shop_id" class="form-control" disabled>
                                    <option value="" hidden>Select Shop</option>
                                    @if($shop->isNotEmpty())
                                    @foreach($shop AS $row)
                                        <option value="{{ $row->id }}" @if ($data->shop_id == $row->id) selected @endif>{{ $row->name }}</option>
                                    @endforeach
                                    @endif
                                </select>
                            ...
                            <div class="form-group col-sm-6">
                                ...
                                <input type="text" name="price" id="price" class="form-control digit" placeholder="Plese enter price" value="{{  $data->price ?? ''  }}" disabled>
                                <span class="kt-form__help error price"></span>
                            </div>
                            <div class="form-group col-sm-6">
                                <label for="stock">Stock</label>
                                <input type="text" name="stock" id="stock" class="form-control digit" placeholder="Plese enter stock" value="{{  $data->stock ?? ''  }}" disabled>
                                <span class="kt-form__help error stock"></span>
                            </div>
                            <div class="form-group col-sm-12">
                                <label for="video">Video</label>
                                <input type="file" class=" dropify" id="video" name="video" data-default-file="{{  $data->video ?? ''  }}" accept="video/mp4,video/x-m4v,video/*" data-allowed-file-extensions='["mp4", "3gp" ,"mkv"]' data-max-file-size-preview="5M" data-show-remove="false" disabled>
